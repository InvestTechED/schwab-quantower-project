...
internal sealed class SchwabMarketDataVendor : Vendor
{
    ...
    public override IList<MessageTrade> GetTrades(TradesHistoryRequestParameters parameters)
    {
        ...
        var trades = this.backendClient.GetTradesAsync(parameters.From, parameters.To, parameters.CancellationToken)
            .GetAwaiter()
            .GetResult();
        this.MarkBackendSuccess();
    ...
    private static MessageTrade CreateTrade(BrokerTradeDto trade) =>
        new()
        {
            ...
            Side = IsBuyInstruction(trade.Instruction) ? Side.Buy : Side.Sell,
