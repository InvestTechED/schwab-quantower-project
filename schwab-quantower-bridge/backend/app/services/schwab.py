...
class SchwabMarketDataService:
    def __init__(self) -> None:
        ...
        self._inflight_requests: dict[tuple[str, object], Future] = {}
    ...
    def get_snapshot(self, symbol: str) -> MarketSnapshot:
        ...
        def build_snapshot() -> MarketSnapshot:
            ...
            self._snapshot_cache[requested_symbol] = (datetime.now(timezone.utc), snapshot)
            return snapshot

        try:
            return self._run_singleflight(("snapshot", requested_symbol), build_snapshot)
        except HTTPStatusError as exc:
            logger.exception("get_snapshot failed for symbol=%s", requested_symbol)
            if cached_snapshot:
                return cached_snapshot[1]
            if exc.response is not None and exc.response.status_code == 400:
                logger.warning("Returning empty snapshot fallback for invalid symbol=%s", requested_symbol)
                fallback = self._empty_snapshot(requested_symbol)
                self._snapshot_cache[requested_symbol] = (datetime.now(timezone.utc), fallback)
                return fallback
            raise
        except Exception:
            logger.exception("get_snapshot failed for symbol=%s", requested_symbol)
    ...
    def get_symbol_profile(self, symbol: str) -> SymbolProfile:
        ...
        if not asset_type or not instrument_type:
            try:
                response = self._run_singleflight(
                    ("symbol_profile_quote", requested_symbol),
                    lambda: self._request_with_retry(lambda client: client.get_quote(schwab_symbol)),
                )
                response.raise_for_status()
                payload = _extract_quote_payload(response.json(), schwab_symbol)
    ...
    def _get_screening_snapshots(self, symbols: tuple[str, ...]) -> dict[str, MarketSnapshot]:
        ...
        for chunk in _chunked(symbols_to_fetch, 40):
            try:
                response = self._run_singleflight(
                    ("screen_quotes", tuple(chunk)),
                    lambda: self._request_with_retry(
                        lambda client: client.get_quotes([normalize_symbol(symbol) for symbol in chunk])
                    ),
                )
                response.raise_for_status()
                payload = response.json()
    ...
    def _empty_snapshot(self, requested_symbol: str) -> MarketSnapshot:
        now = datetime.now(timezone.utc)
        return MarketSnapshot(
            symbol=requested_symbol,
            as_of=now,
    ...
    def get_option_series(self, symbol: str) -> list[OptionSeries]:
        ...
        response = self._run_singleflight(
            ("option_series", requested_symbol),
            lambda: self._request_with_retry(lambda client: client.get_option_expiration_chain(normalize_symbol(requested_symbol))),
        )
        response.raise_for_status()
        payload = response.json()
    ...
    def get_option_chain(self, symbol: str, expiration: date | None = None) -> OptionChain:
        ...
        response = self._run_singleflight(
            ("option_chain", requested_symbol, expiration_key),
            lambda: self._request_with_retry(
    ...
    def get_bars(
        ...
        start_key = self._history_cache_key_time(timeframe_key, normalized_start)
        end_key = self._history_cache_key_time(timeframe_key, normalized_end)
        ...
        try:
            response = self._run_singleflight(
                ("bars", requested_symbol, timeframe_key, limit, start_key, end_key),
                lambda: self._request_with_retry(
                    lambda client: self._get_price_history_response(
                        client,
                        schwab_symbol,
                        timeframe_key,
                        normalized_start,
    ...
    def _history_cache_key_time(self, timeframe: str, value: datetime | None) -> str | None:
        if value is None:
            return None

        current = self._as_utc(value)
        if timeframe == "1m":
            current = current.replace(second=0, microsecond=0)
        elif timeframe in {"5m", "10m", "15m", "30m"}:
            minutes = int(timeframe[:-1])
            bucket_minute = (current.minute // minutes) * minutes
            current = current.replace(minute=bucket_minute, second=0, microsecond=0)
        elif timeframe in {"1h", "4h"}:
            bucket_minute = 30 if current.minute >= 30 else 0
            current = current.replace(minute=bucket_minute, second=0, microsecond=0)
        else:
            current = current.replace(second=0, microsecond=0)
    ...
    def _run_singleflight(self, key: tuple[str, object], operation):
        created = False
