...
class Settings:
    def __init__(self) -> None:
        ...
        self.schwab_fallback_account_number = os.getenv("SCHWAB_FALLBACK_ACCOUNT_NUMBER", "")
        self.schwab_trading_enabled = os.getenv("SCHWAB_TRADING_ENABLED", "true").lower() in {"1", "true", "yes", "on"}
        # Zero or a negative value disables the share-count cap while preserving
        # the other trading guardrails.
        self.schwab_max_order_shares = float(os.getenv("SCHWAB_MAX_ORDER_SHARES", "1"))
        # Zero or a negative value disables the notional cap.
        self.schwab_max_order_notional = float(os.getenv("SCHWAB_MAX_ORDER_NOTIONAL", "0"))
